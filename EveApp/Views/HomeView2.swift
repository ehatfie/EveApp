...
@Observable class HomeViewModel2 {
    ...
    func checkForNeedsAuthentication() {
        ...
        print(
            "checkForNeedsAuthentication: \(needsAuthentication && !needsAuthSetup)"
        )
    }
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem {
                HStack {
                    Text("Listener")
                    Toggle(isOn: $modelData.listenerEnabled, label: {
                        Text("")
                    })
                    .toggleStyle(.switch)//.frame(maxWidth: .infinity)
                    .onChange(of: modelData.listenerEnabled, {
                        // this could be better
                        modelData.updateListener()
                    })
                }.padding(.horizontal)
            }
        }
